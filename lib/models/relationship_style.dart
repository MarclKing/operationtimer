...
class RelationshipTexts {
  ...
  static NotificationCopy dailyOverview({
    ...
    // 1) HEADER — simpler Dienst-Hinweis, keine Stil-Varianten.
    var title = phrases.simpleShiftHeader(
      hasShift: hasShift,
      isFree: isFree,
      shiftCode: shiftCode,
    );
    ...
    // 3) LUSTIGER SPRUCH (bisherige "subtitle"-Phrasen)
    final joke = phrases.applyName(pick(phrases.subtitle(style)), name);
    lines.add(joke);
    ...
    final taskOptions = phrases.taskLine(
      ...
      dueTodayTaskTitle: dueTodayTaskTitle,
