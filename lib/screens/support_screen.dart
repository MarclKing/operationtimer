...
class _SupportScreenState extends State<SupportScreen> {
  ...
  Future<void> _sendSupport() async {
    if (_messageController.text.trim().isEmpty) {
      showGlassSnackBar(context, 'Bitte eine Nachricht eingeben', type: GlassSnackBarType.error);
      return;
    }
    ...
    if (await canLaunchUrl(uri)) {
      ...
      if (mounted) {
        showGlassSnackBar(context, 'Keine Mail-App gefunden', type: GlassSnackBarType.error);
      }
    }
  ...
  Future<void> _submitBug() async {
    ...
    if (title.isEmpty) {
      showGlassSnackBar(context, 'Bitte einen Titel eingeben', type: GlassSnackBarType.error);
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        showGlassSnackBar(context, 'Bug gemeldet – Danke!', type: GlassSnackBarType.success);
      }
    } catch (e) {
      if (mounted) {
        showGlassSnackBar(context, 'Fehler: $e', type: GlassSnackBarType.error);
      }
    }
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      GlassPrimaryButton(
                        skin: skin,
                        large: true,
                        icon: _sending ? null : Icons.email_outlined,
                        label: _sending ? 'Wird gesendet…' : 'Nachricht senden',
                        onTap: _sending ? () {} : _sendSupport,
                      ),
                      ...
                      Center(
                        child: Text(
                          ...
                          style: TextStyle(fontSize: 12, color: skin.surface(0.28)),
                        ),
                      ),
                      ...
                      // ── Screenshot Picker ──────────────────────────────────────
                      GlassInfoCard(
                        icon: _bugScreenshot != null ? Icons.image_rounded : Icons.add_photo_alternate_outlined,
                        iconColor: _bugScreenshot != null ? skin.primary : skin.surface(0.4),
                        iconBgColor: _bugScreenshot != null ? skin.primary.withValues(alpha: 0.10) : skin.surface(0.06),
                        title: _bugScreenshot != null ? 'Screenshot ausgewählt' : 'Screenshot hinzufügen',
                        description: _bugScreenshot != null ? 'Tippe zum Ändern oder entferne es rechts.' : 'Optional – hilft uns, den Bug schneller zu verstehen.',
                        onTap: _pickScreenshot,
                        trailing: _bugScreenshot != null
                            ? GestureDetector(
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: skin.deleteColor.withValues(alpha: skin.isLight ? 0.10 : 0.18),
                            borderRadius: BorderRadius.circular(16),
                            border: Border.all(color: skin.deleteColor.withValues(alpha: 0.35), width: 1.3),
                          ),
                          child: Center(child: _submittingBug
                              ? SizedBox(width: 18, height: 18,
                                  child: CircularProgressIndicator(strokeWidth: 2, color: skin.deleteColor))
                              : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                                  Icon(Icons.bug_report_outlined, color: skin.deleteColor, size: 18),
                      ...
                      if (AuthService.instance.isAdmin) ...[
                        ...
                        GestureDetector(
                          onTap: () => Navigator.push(context,
                              CupertinoPageRoute(builder: (_) => const BugAdminScreen())),
                          child: Container(
                            width: double.infinity,
